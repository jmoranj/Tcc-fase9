...
export default function AlbumInfo({ route }) {
        ...
        <ScrollView style={{ marginTop: 50 }}>
          ...
          <View style={{ marginTop: 10, marginHorizontal: 12 }}>
            {tracks.map((track, index) => (
              <Pressable isPlaying={item === currentTrack} key={index} onPress={() => play(track, index)} style={styles.songContainer}>
                <View style={styles.songInfoContainer}>
                  <View style={styles.textContainer}>
                    <Text numberOfLines={1} style={currentPlayingIndex === index ? styles.playingTitle : styles.title}>
                      {track?.name}
                    </Text>
                    {track?.artists?.map((item, artistIndex) => (
